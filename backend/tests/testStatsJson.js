...
async function testStats() {
    ...
    try {
        ...
        fs.writeFileSync(path.join(__dirname, '../data/stats_result.json'), JSON.stringify(results, null, 2));
        process.exit(0);
    } catch (err) {
        fs.writeFileSync(path.join(__dirname, '../data/stats_result.json'), JSON.stringify({ error: err.message }, null, 2));
        process.exit(1);
    }
}
