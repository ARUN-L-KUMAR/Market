...
import { getProductById, updateProduct, uploadProductImage } from '../../services/adminService';
    ...
    const fetchProduct = async () => {
        try {
            ...
            if (product.category) {
                ...
                try {
                    const subResponse = await apiClient.get(`/api/categories?parent=${catId}`);
                    ...
                    <div className="flex justify-between items-center mb-8">
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all text-slate-400 hover:text-slate-600"
                            >
                                <ArrowLeft className="h-5 w-5" />
                            </button>
                            <div>
                                <h1 className="text-2xl font-bold text-slate-900">Edit Product</h1>
                                <p className="text-sm text-slate-500 mt-1">Update the details of your existing product.</p>
                            </div>
                        </div>
